...
class XM_Cart {
	/**
	 * Does any necessary functionality before any cart functionality can be used.
	 * This includes:
	 *
	 *   - make sure the salt is set and more than 32 characters
	 *
	 * @return  void
	 */
	public static function load() {
		$salt = Cart_Config::load('salt');
		if (empty($salt)) {
			throw new Kohana_Exception('The cart salt has not been set');
		} else if (UTF8::strlen($salt) < 32) {
			throw new Kohana_Exception('The cart salt must be longer than 32 characters');
		}
	}
	...
	/**
	 * Returns TRUE if the order can be edited by a customer.
	 * The order must have the status of new or submitted.
	 *
	...
	/**
	 * Returns TRUE if the order can be viewed by a customer.
	 * The order must have the status of paid, received or shipped.
	 *
	 * @param   Model_Cart_Order  $order  The order model.
	 *
	 * @return  boolean
	 */
	public static function allow_order_view($order) {
		return in_array((int) $order->status, array(CART_ORDER_STATUS_PAID, CART_ORDER_STATUS_RECEIVED, CART_ORDER_STATUS_SHIPPED), TRUE);
	}
	...
	public static function send_customer_order_email($order, $order_payment, $order_message = NULL) {
		...
		if ($have_customer_email) {
			$email_body_html = View::factory('cart/email/customer_order')
				...
				->set('view_order_url', Cart::order_view_url($order))
	...
	public static function send_admin_order_email($order, $order_payment) {
		...
		$admin_view_order_url = URL::site(Route::get('cart_admin_order')->uri(array('action' => 'view', 'id' => $order->pk())));
		...
		$email_body_html = View::factory('cart/email/admin_order')
			...
			->bind('paid_with', $paid_with)
			->bind('cart_view', $email_cart_view)
			->bind('admin_view_order_url', $admin_view_order_url)
			->set('enable_shipping', Cart_Config::enable_shipping())
			->set('enable_tax', Cart_Config::enable_tax())
			->set('donation_cart', $is_donation_cart);
	...
	/**
	 * Generates the full URL to view the order, including the encrypted order key.
	 * Used in emails to link back to the order.
	 *
	 * @param   Model_Cart_Order  $order  The order model.
	 *
	 * @return  string
	 */
	public static function order_view_url($order) {
		// generate the view link by encrypting some of the order data
		$order_key_data = array(
			'order_id' => $order->pk(),
			'unique_id' => $order->unique_id,
			'user_id' => $order->user_id,
		);
		$order_key = Encrypt::instance('xm_cart')
			->encode(json_encode($order_key_data));
		$view_order_query_str = http_build_query(array('order' => $order_key));
		return URL::site(Route::get('cart_public')->uri(array('action' => 'view_order')) . '?' . $view_order_query_str);
	}

	/**
	 * Takes the order key from the url/query string generated by Cart::order_view_url() and attempts to retrieve the associated order.
	 * May throw an exception from the invalid JSON, missing `order_id` or `unique_id`.
	 *
	 * [!!] Make sure the order model is loaded before using it.
	 *
	 * @param   string  $order_key  The `order` query string value from Cart::order_view_url().
	 *
	 * @return  Model_Cart_Orders
	 */
	public static function order_key_decrypt($order_key) {
		...
		if (empty($order_data['order_id']) || empty($order_data['unique_id'])) {
			throw new Kohana_Exception('The proper data was not received to view the order');
		}

		return ORM::factory('Cart_Order', array(
			'id' => $order_data['order_id'],
			'unique_id' => $order_data['unique_id'],
